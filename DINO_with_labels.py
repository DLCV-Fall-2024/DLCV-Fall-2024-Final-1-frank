...
# Define directories
OUTPUT_DIR = "DINO_with_information_results"
...
def process_and_save_results(image, image_id, processor, groundingdino_model, depth_model, device, output_dir):
    ...
    annotated_image_path = os.path.join(output_dir, f"{image_id}_annotated.jpg")
    ...
    save_detection_results(image_id, boxes, labels, depths, output_dir)
...
if __name__ == "__main__":
    ...
    for data in dataset:
        ...
        # Filter to only process "regional" or "general" image IDs
        if "regional" in image_id.lower() or "general" in image_id.lower():
            print(f"Processing image ID: {image_id}")
            process_and_save_results(image, image_id, processor, groundingdino_model, depth_model, device, OUTPUT_DIR)
        else:
            print(f"Skipping image ID: {image_id} (not 'regional' or 'general')")
