...
def load_detection_model():
    model = YOLO('checkpoints/YOLO/yolov8x.pt').to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    return model
...
def process_but_no_save_segmentation(image, sam_predictor, detection_model):
    image_np = np.array(image)
